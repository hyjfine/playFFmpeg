...
static AVCodecContext *create_decoder(enum AVCodecID codec_id) {
    ...
    if (codec == NULL) {
        LOGD("ERROR: avcodec_find_decoder failed");
        return NULL;
    }

    codec_context = avcodec_alloc_context3(codec);
    if (codec_context == NULL) {
        LOGD("ERROR: avcodec_alloc_context3 failed");
        return NULL;
    }
...
static int open_audio_decoder(Player *player) {
    ...
    if (ret < 0) {
        LOGD("ERROR: avcodec_open2 failed: %s", av_err2str(ret));
    }
    return ret;
}
...
static int convert_video_frame(Player *player, AVFrame *frame) {
    ...
    if (scale_context == NULL) {
        LOGD("ERROR: sws_getContext failed");
        return AVERROR(ENOMEM);
    }

    uint8_t *dst_data[8] = {NULL};
    int dst_linesize[8] = {0};
    int ret = av_image_alloc(dst_data, dst_linesize, frame->width, frame->height, AV_PIX_FMT_RGBA,
                             1);
    if (ret < 0) {
        LOGD("ERROR: av_image_alloc failed: %s\n", av_err2str(ret));
...
static int decode(Player *player, AVCodecContext *codec_context, AVPacket *packet) {
    ...
    if (ret < 0) {
        LOGD("ERROR: avcodec_send_packet failed: %s\n", av_err2str(ret));
    }
    while (ret >= 0) {
        ...
        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
            ...
            LOGD("ERROR: avcodec_receive_frame failed: %s\n", av_err2str(ret));
            break;
        }

        LOGD("index: %d pts: %lld dts: %lld duration: %lld\n", codec_context->frame_number,
